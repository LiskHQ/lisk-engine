...
func (ui *ChatUI) Run(addrs []string) error {
...
func (ui *ChatUI) displayChatMessage(cm *ChatMessage) {
	if isPublishedByMe(cm.Message) {
		return
	}
	prompt := withColor("blue", fmt.Sprintf("-%s:", cm.SenderNick))
	fmt.Fprintf(ui.msgW, "%s %s\n", prompt, cm.Message)
}

// displaySelfMessage writes a message from ourself to the message window,
// with our nick highlighted in yellow and red for public message.
func (ui *ChatUI) displaySelfMessage(msg string) {
	var prompt string
	if strings.HasPrefix(msg, PublicPrefix) {
		prompt = withColor("green", fmt.Sprintf(":%s", msg))
		msg = ""
	} else if res, ok := removePrefix(msg); ok && strings.HasPrefix(msg, ErrorPrefix) {
		msg = res
		prompt = withColor("red", fmt.Sprintf(":%s", msg))
		msg = ""
	} else {
		if res, ok := removePrefix(msg); ok && strings.HasPrefix(msg, P2pPrefix) {
			msg = res
		}
...
// removePrefix removes first part of msg if it has specific prefix.
func removePrefix(msg string) (string, bool) {
	prefix := ""
	if strings.HasPrefix(msg, P2pPrefix) {
		prefix = P2pPrefix
	} else if strings.HasPrefix(msg, ErrorPrefix) {
		prefix = ErrorPrefix
	}
	if prefix != "" {
		res := strings.Replace(msg, prefix, "", 1)
		return res, true
	}
...
// isPublishedByMe return true if the message published by itself.
func isPublishedByMe(msg string) bool {
	_, ok := removePrefix(msg)
	return ok || strings.HasPrefix(msg, PublicPrefix)
}
...
func withColor(color, msg string) string {
	return fmt.Sprintf("[green][%s][-][%s]%s[-]", time.Now().Format(time.StampMilli), color, msg)
}
